...
function CategoryPage({ match }) {
    ...
    const [urlparams, seturlparams] = useState(`${absolute_url}/category/${match.params.category_name}?limit=10&skip=0`)
        ...
        const apiCall = async () => {
            ...
            try {
                ...
                if (matchFromUrl !== match.params.category_name) {
                    ...
                    seturlparams(`${absolute_url}/category/${match.params.category_name}?limit=10&skip=0`)
                ...
                if (data.products) {
                    ...
                    setnexturl((prevstate) => {
                        if (data.nextskip) {
                            return `${absolute_url}/category/${match.params.category_name}?limit=10&skip=${data.nextskip}`
                        } else {
                            return null
                        }
                    })
                    setprevurl((prevstate) => {
                        if (data.prevskip || data.prevskip == 0) {
                            return `${absolute_url}/category/${match.params.category_name}?limit=10&skip=${data.prevskip}`
                        } else {
                            return null
                        }
                    })
